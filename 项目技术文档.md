...
接入工程需要自行提供日志框架与密码学 Provider 依赖（本组件的 `slf4j/jackson/logback/log4j2` 在 POM 中为 provided）。
...
- Log4j2：`org.apache.logging.log4j:log4j-api`、`org.apache.logging.log4j:log4j-core`（如需走 SLF4J：`org.apache.logging.log4j:log4j-slf4j-impl`）

### 2. 准备配置文件（securelog-ecc.properties）

在接入工程的 classpath（例如 `src/main/resources`）放置 `securelog-ecc.properties`，最少需要配置：
...
- `ecc.crypto.provider` 必须是 Provider 类全路径；需要确保对应 Provider 依赖已引入。
- 其余配置可不写，组件会走默认值（如 `SM4/GCM/NoPadding`、脱敏 key 列表、MDC key 等）。

### 3. 配置日志框架接入

#### 3.1 Logback（logback.xml）

用 `SecureMaskingAppender` 包装你的目标 Appender：
...
<configuration>
  <appender name="FILE" class="ch.qos.logback.core.FileAppender">
    ...
    <encoder class="ch.qos.logback.classic.encoder.PatternLayoutEncoder">
      <pattern>%msg%n MDC_SECURE_DATA=%X{SECURE_DATA}%n</pattern>
    </encoder>
  </appender>
...
通过 RewritePolicy 注入脱敏与 `SECURE_DATA`，并配置 `packages` 以加载插件：
...
- 日志正文中敏感值已脱敏（结构化优先：JSON/SQL Parameters/URL query/querystring/key-value/纯文本兜底）。
- 同一事件的上下文字段中带有 `SECURE_DATA`（Base64），可用于审计回溯解密。
...
也可以自建入口调用 `team.frog.securelogecc.cli.Sm2CliApp` 生成 SM2 密钥对/解密 SECURE_DATA。
...
- **ConcurrentLinkedQueue**：创建顺序队列，按创建顺序清理
...
- **作用**：提供敏感数据检测和加密功能
- **自动检测**：基于结构化优先脱敏引擎识别敏感数据（JSON/SQL Parameters/URL query/querystring/key-value/纯文本兜底）；trace_id 仅从 MDC 的配置 key 列表读取
...
1. **消息格式化**：先由日志框架完成消息参数格式化（例如 `getFormattedMessage()`）
2. **敏感数据处理**：对消息文本进行结构化优先脱敏并提取原始敏感值→拼装JSON→生成SECURE_DATA
3. **写入上下文**：将 SECURE_DATA 写入“事件上下文”（Logback: event MDCPropertyMap；Log4j2: event ContextData）
4. **输出脱敏消息**：将脱敏后的消息交给下游 encoder/layout 按 pattern 组装整行日志（脱敏结果不一定总是 `***`，可能是部分保留/格式化）
...
1. **结构化优先脱敏**：按固定顺序处理消息文本：JSON → SQL Parameters → URL query → querystring → key/value 对 → 纯文本兜底
2. **提取与JSON转换**：将提取到的“原始敏感值”键值对转换为JSON字符串
3. **trace_id识别**：仅从 MDC 的配置 key 列表（`mdc.trace.id.keys`）读取，未命中视为无 trace_id
4. **加密处理**：trace_id 存在则生成“会话密钥”SECURE_DATA，否则生成“系统密钥”SECURE_DATA
5. **返回结果**：返回脱敏后的消息文本；`processLog()` 会按配置 Key 将 SECURE_DATA 写入/移除线程 MDC，`processLogResult()` 仅返回结果不写 MDC
...
// 创建顺序队列：ConcurrentLinkedQueue
private final ConcurrentLinkedQueue<String> sessionAccessQueue=new ConcurrentLinkedQueue<>();
private final ConcurrentLinkedQueue<String> systemAccessQueue=new ConcurrentLinkedQueue<>();
```
...
- **极致性能**：命中直接返回；缺失时使用原子创建减少竞争
- **代码简洁**：逻辑清晰，易于维护
- **并发安全**：避免并发重复生成同一 key 的密钥
...
- SQL Parameters 日志（形如 `Parameters: ...`）：对所有 `(String)` 参数一律脱敏；命中身份证/手机号/邮箱/严格地址时按对应规则格式化，否则替换为 `***`。
...
`SECURE_DATA` 是一个 Base64 编码的字节数组，采用混合加密方案，包含以下结构：

```
[version(1)] + [SM2密钥长度(4)] + [IV长度(1)] + [SM2加密的密钥] + [IV] + [SM4加密的敏感数据]
```
...
### 4. 控制台工具（SM2密钥生成与SECURE_DATA解密）

#### 入口

- 需要在任意工程中自建入口类调用：`new team.frog.securelogecc.cli.Sm2CliApp().runCli();`

#### 运行

```bash
cd e:\trae\LogSecure\SecureLog-ECC
...
set CLASSPATH=target\<你的工程产物>.jar;@cp.txt
java -cp %CLASSPATH% <你的入口类全限定名>
```

#### 功能

- 生成SM2公钥/私钥（Base64），并保存到当前目录：`sm2_public_key_时间戳.txt`、`sm2_private_key_时间戳.txt`
- 解密SECURE_DATA（Base64），输出明文，并追加保存到：`sm2_decrypt_output.txt`
- 使用前请确保引入密码学提供者依赖（如 BouncyCastle Provider）

#### 使用流程

1. 运行CLI后选择操作：`1` 生成密钥对，`2` 解密SECURE_DATA
2. 解密时输入私钥（Base64），然后输入SECURE_DATA（Base64）
3. 可输入 `c` 继续解密下一条，其他输入退出

### 5. 日志框架集成

#### 执行顺序与MDC规则（重要）

- **执行顺序固定**：适配层对“已完成参数替换的消息文本”（如 `getFormattedMessage()`）做脱敏与 SECURE_DATA 生成；随后由下游 encoder/layout 按
  pattern/ConversionPattern 组装整行日志。
- **SECURE_DATA 的写入位置**：适配层将 SECURE_DATA 写入事件上下文（Logback: event MDCPropertyMap；Log4j2: event
  ContextData）；若直接调用 `LogMaskingProcessor.processLog()`，则按配置 Key 写入线程 MDC。
- **避免串值**：事件上下文随事件传递，不会跨事件串值；线程 MDC 场景每次调用都会 put/remove 覆盖，不会残留旧值。
...
- **并发性能**：读路径无显式锁；创建路径使用 `computeIfAbsent` 降低竞争
...
- **密钥分离**：会话密钥和系统密钥完全隔离
- **密钥加密**：SM4密钥使用SM2加密后写入SECURE_DATA载荷
- **密钥轮换**：系统密钥按时间间隔自动轮换

### 2. 数据安全

- **敏感数据检测**：结构化优先识别（JSON/SQL Parameters/URL query/querystring/key-value/纯文本兜底）+ 值形态命中
- **完整加密**：敏感数据和密钥都进行加密
- **加密模式**：默认 `SM4/GCM/NoPadding`（随机IV/nonce + 认证标签），可通过配置项调整

### 3. 配置安全

- **配置编码**：ECC公钥以Base64编码存储（编码非加密）
- **默认安全**：提供安全的默认配置值
- **配置校验**：对部分关键配置做范围/必填校验（如缓存百分比范围、公钥必配、提供者可用性）
